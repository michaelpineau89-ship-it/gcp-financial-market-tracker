...
def run_fmp_ingestion():
    ...
    try:
        if income_master:
            df_inc = pd.DataFrame(income_master).assign(_ingested_at = pd.Timestamp.utcnow())
            pandas_gbq.to_gbq(df_inc, "market_tracker.bronze_fmp_income", project_id=PROJECT, if_exists="append")
            logging.info(f"Loaded {len(df_inc)} rows to Income table.")
            
        if balance_master:
            df_bal = pd.DataFrame(balance_master).assign(_ingested_at = pd.Timestamp.utcnow())
            pandas_gbq.to_gbq(df_bal, "market_tracker.bronze_fmp_balance", project_id=PROJECT, if_exists="append")

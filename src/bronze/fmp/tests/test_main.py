...
class TestRunFmpIngestion:
    ...
    def test_run_fmp_ingestion_http_success(self, mock_impl):
        ...
        response_body, status_code, headers = run_fmp_ingestion(mock_request)
    ...
    @patch("main.run_fmp_ingestion_impl")
    def test_run_fmp_ingestion_http_with_request_data(self, mock_impl):
        """Test HTTP handler with request data"""
        mock_impl.return_value = {"status": "success", "code": 200, "message": "OK"}

        # Create mock HTTP request with JSON data
        mock_request = Mock()
    ...
    @patch("main.run_fmp_ingestion_impl")
    def test_run_fmp_ingestion_http_error(self, mock_impl):
    ...
    @patch("main.run_fmp_ingestion_impl")
    def test_run_fmp_ingestion_http_successful_response(self, mock_impl):
        """Test HTTP handler returns proper response format"""
        mock_impl.return_value = {"status": "success", "code": 200, "message": "OK"}
